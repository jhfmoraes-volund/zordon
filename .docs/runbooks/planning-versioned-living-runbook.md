...
## 17. Rev. 6 — Apply-flow hardening (crítica externa, 2026-06-20)

Resposta a uma auditoria do fluxo de Aplicar (8 achados). **6 corrigidos:**
- **#1 skipped visível:** toast do apply agora mostra `X aplicadas · Y puladas (trabalho em curso ou duplicata) · Z falhou` (antes só applied+failed → puladas sumiam sem explicação). Severidade do toast por resultado (error se 0 aplicado, warning se houve pulada/falha).
- **#2 chat só reseta se aplicou:** `handleApplied(result)` só abre chat novo (próxima versão) quando `applied>0`. Apply que não aplicou nada (tudo falhou/pulado) PRESERVA o papo — é quando o PM precisa do contexto pra pedir conserto. `onApplied` agora recebe `{applied,failed,skipped}`.
- **#5 descartadas consumidas:** ao concluir, `applyPendingActionsForPlanning` marca as `decision=rejected, execution=pending` como `execution=skipped` (não inflam `result.skipped`) — antes reapareciam riscadas em todo ciclo (dívida de staging). Verificado e2e.
- **#6 reclique empilha:** "Montar plano" confirma quando já há staging pendente ("considere aplicar primeiro").
- **#7 fontes vazias:** "Montar plano" confirma quando não há insumo NEM PRD linkado (Vitoria partiria do zero → risco de inventar).
