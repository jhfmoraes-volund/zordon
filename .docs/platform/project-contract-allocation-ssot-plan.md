...
| **D11** | **Participação pontual (spot) = `labor_allocation` com `kind='spot'`, medida em DIAS (1 dia = 8h), sem `%`.** Admin informa `days` (fracionável, ex. 2,5). Custo = `days × custo-dia` (comp mensal ÷ dias úteis) — aproximação aceita ("me dá o valor"). Mesmo SSOT, mesma interface admin. **REVISADO 2026-06-23** (antes era sprint+%). | João: unidade dia é mais simples de gerenciar e dá o valor; % por sprint era espelho desnecessário. |
| **D12** | **Teto = 60 dias POR ENTRADA de spot** (cada entrada nasce com seu próprio limite; a mesma pessoa pode ter N entradas no projeto). Atribuição de custo: mês da data de início da entrada (default; refinar se precisar dividir entre meses). | Limite por entrada (não por membro×projeto) — João escolheu; mantém cada ajuda como unidade independente. |
| **D13** | **Spot = builder normal pra acesso: `ProjectAccess` contributor PERMANENTE (não expira).** Acessa tudo como builder mesmo depois; "pronto pra voltar". Reativar = nova entrada de dias. **REVISADO** (antes era acesso só pela vigência). | João: mais fácil de gerenciar + deixa preparado pra voltar. "O user mesmo sendo SPOT tem acesso a tudo como builder normal." |
| **D14** | **"Tipos de contrato" = os kinds do projeto** (Interno/Proposta/Contratado, D4). Sem campo "tipo" novo no contrato — `billing_type` + `status` + kind do projeto cobrem. | Confirmado por João. |
