...
> **Falta:** (a) `CRON_SECRET` + `ZORDON_URL` no `.env` do daemon (pro `recompose_wiki` alcançar `/api/internal/wiki-composer`); (b) **restart do daemon** (sua máquina) + commit do repo do daemon; (c) verify no browser: abrir um projeto → chat do Alpha → "ajusta a wiki: destaca X" → recompose. A surface 'wiki' da Vitoria fica pronta (inerte) pra um futuro chat no sheet, sem custo.
