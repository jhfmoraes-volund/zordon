...
| 2026-06-24 | **F2.1.1 task sheets convergidos** | ✅ resíduo (1) fechado. Gate de `/api/projects/[id]/members` relaxado `MANAGER`→`requireProjectViewApi` (manager OU ProjectAccess — espelha RLS de SELECT de ProjectMember; superset, nada quebra). 2 loaders de task sheet (`task-sheet-by-ref.tsx` + `story-hierarchy/use-task-sheet-context.ts`) agora dão fetch em /members (v_project_team) no lugar do UNION client-side; task-sheet-by-ref preserva assignees atuais ausentes do roster (chip não some). Bônus: corrigido setState-in-effect pré-existente em use-task-sheet-context. grep ProjectMember/ProjectSquad/SquadMember nos 2 = vazio; tsc 0; eslint 0 erros. **Resta:** (2) route.ts memberCapacity (PFV/D10), (3) finance dal squad, (4) vitoria profile (pool, ok), + Alpha set_fp_allocation update-only. |
