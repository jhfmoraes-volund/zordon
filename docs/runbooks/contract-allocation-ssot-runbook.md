...
| 2026-06-24 | **F2.1.1 task sheets convergidos** | ✅ resíduo (1) fechado. Gate de `/api/projects/[id]/members` relaxado `MANAGER`→`requireProjectViewApi` (manager OU ProjectAccess — espelha RLS de SELECT de ProjectMember; superset, nada quebra). 2 loaders de task sheet (`task-sheet-by-ref.tsx` + `story-hierarchy/use-task-sheet-context.ts`) agora dão fetch em /members (v_project_team) no lugar do UNION client-side; task-sheet-by-ref preserva assignees atuais ausentes do roster (chip não some). Bônus: corrigido setState-in-effect pré-existente em use-task-sheet-context. grep ProjectMember/ProjectSquad/SquadMember nos 2 = vazio; tsc 0; eslint 0 erros. |
| 2026-06-24 | **F2.1.2 go-forward PM no roster** | ✅ migration 20260624p PROD: trigger `trg_sync_pm_project_access` (AFTER INSERT OR UPDATE OF pmId em Project) concede ProjectAccess('lead') ao PM via Member.userId (SECURITY DEFINER, idempotente ON CONFLICT, pula sem userId). Fecha o buraco: projeto NOVO / pmId trocado deixaria o PM fora do v_project_team (a backfill F2.7 só cobriu os de hoje). Acesso≠custo (não cria allocation). Verificado (rollback): setar pmId fresco → PM aparece is_pm=t source=access role=lead. |
| 2026-06-24 | **Resíduos reavaliados** | (3) **Alpha set_fp_allocation NÃO precisa virar update-only**: pós-F2.8 v_project_team NÃO lê ProjectMember como fonte de membership (só LEFT JOIN p/ fp_allocation/teto PFV), então INSERT de ProjectMember pelo Alpha não escreve roster — invariante preservada sem mexer no tool (regra: não tocar tool de agente sem capture). (2) `route.ts` memberCapacity + (4) `vitoria/profile.ts` = domínio PFV/contexto-pool (D9/D10), mantidos de propósito. Convergência de roster: **completa**. |
| 2026-06-24 | **Editar projeto migra pro S&OP + zonas Projeto×Contrato** | Definição cravada: **Projeto (1, container/identidade) CONTÉM Contratos (N, comercial)** — `contract.project_id`+`seq`. category/phase/status = do projeto; vigência/billing/valor/status-comercial/equipe = do contrato; datas do projeto DERIVAM do contrato (trigger), próprias só sem contrato. **UI (sem schema):** (1) `FinanceProjectView` ganhou zona **PROJETO** (chips phase/status/category + "Editar projeto" abrindo o ProjectEditSheet edit, carregando a row via supabase) acima da zona **CONTRATOS deste projeto**; (2) empty-states por categoria (interno = "não precisa de contrato" · billable = "+ Contrato"); (3) **edição saiu da lista de projetos** (só criar lá), vai pro S&OP; (4) handoff pós-criação → `/?tab=apps&app=finance&fp=<id>` (FinanceApp auto-seleciona o projeto). Mockups: /tmp/new-project-flow.html, /tmp/snop-project-spaces.html. tsc 0, eslint 0. |
| 2026-06-24 | **UX criação de projeto (admin-only, sem duplicação)** | Decisão: abertura de projeto centralizada no admin; sheet só cria **identidade** (Nome, Tipo, Cliente, PM, Drive), e **contrato + equipe** vivem só no S&OP (Finanças) — zero duplicação. project-edit-sheet: criação enxuta (Nome em 1º; team/repo/github/engajamento/datas/status/fase/categoria viram edit-only via `{project && …}`); save() cria contrato STUB (billing 'squad' placeholder, vigência hoje, sem datas do form); **handoff** = ConfirmDialog pós-criar pergunta "Configurar no S&OP?" → deep-link `/projects/[id]?tab=apps&app=contract` (Interno fecha direto, sem contrato). projects-view: botão Novo Projeto + edit (tabela + card mobile) gated `isAdmin`. Mockup fiel: /tmp/new-project-flow.html. tsc 0, eslint 0. |
| 2026-06-24 | **Refino de coerência: EQUIPE = PM + Builders** | Decisão do João: **membro do projeto = PM (gestor, derivado de Project.pmId) + Builders (executores, labor_allocation)**; **guest/viewer NÃO é membro** (ProjectAccess = só visibilidade, fora da equipe). Simplificação (sem feature nova): migration 20260624q redefine `v_project_team` (PM ∪ allocated; remove a perna access-only + colunas source/access_role; PM derivado, sempre presente) e **reverte** o trigger sync_pm_project_access (PM não entra mais via access). Helper `project-team.ts` e alpha `get_allocated_project_members` perdem `source`. As 4 rows ProjectAccess('lead') de PM (backfill F2.7) ficam (grant RLS invisível ao roster). Verificado PROD: todo projeto = 1 PM + N builders, guests fora (Zelar 7→2), 0 PMs sumidos. tsc 0, eslint 0. Bônus: removidos emojis (cards de kind, Ganhar proposta, Pontual). |
