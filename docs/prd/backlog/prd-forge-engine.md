...
| D2 | **ForgeTask (DB table) = ForgeStep semântico** — unidade interna do Forge, gerada pelo planner iter-0. **NÃO confundir com `Task` (PM-level, criada pela Vitoria)**. Cada Task da Vitoria que dispara `forge run` produz 1 ForgeRun com N ForgeTasks (ForgeSteps). Rename para `ForgeStep` é follow-up futuro. | Vocabulário ambíguo é a maior dívida hoje. Documentar explicitamente até refactor. |
| D25 | **Hierarquia canônica de 5 níveis**: PRD (Vitor) → Module → UserStory → Task (Vitoria) → ForgeStep (Forge planner). Os 3 níveis intermediários são criados pela Vitoria em **Planning Session one-shot** por projeto. Forge consome **Task** (não PRD, não UserStory) — busca parent chain do DB. | Sem hierarquia clara, Vitoria/Forge competem por escopo. Com hierarquia, cada agente tem fronteira inequívoca. |
| D26 | **Forge input = Task ID** (não Spec.md slug). Builder roda `forge run <task-id>`. Planner fetcha cadeia parent (UserStory → Module → PRD) via Supabase. PRD vive em DB (ProductRequirement table), não em filesystem — D15 (Spec.md em `docs/specs/`) é deprecada em favor de DB-first. | Single source of truth. Vitor escreve via UI Volund (DB), não em .md no disco. Forge consome via DAL existente. |
