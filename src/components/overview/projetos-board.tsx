...
import { Clock, AlertCircle, UserX, FolderKanban, CheckCircle2 } from "lucide-react";
import {
  ResponsiveSheet,
...
function ProjectSheet({ p }: { p: ProjectOverview }) {
            ...
            <div className="flex flex-wrap gap-1.5">
              {p.team.map((m) => (
                <span key={m.id} className="surface-inset px-2 py-1 text-xs">
                  {m.name}
                  {m.position ? (
                    <span className="ml-1 text-muted-foreground">{m.position}</span>
                  ) : null}
                </span>
              ))}
            </div>
...
export function ProjetosBoard({ projects }: { projects: ProjectOverview[] }) {
  ...
  const grouped = useMemo(() => {
    const visible = projects.filter((p) => showEval || !p.isEval);
    ...
    <div className="space-y-6">
      {evalCount > 0 && (
        <div className="flex justify-end">
          ...
          >
            {showEval ? "Esconder" : "Mostrar"} testes/eval ({evalCount})
          </button>
        </div>
      )}
      ...
      {grouped.length === 0 && (
        <div className="surface flex flex-col items-center gap-2 py-12 text-center text-sm text-muted-foreground">
          ...
          Nenhum projeto visível. Ative mostrar testes/eval acima.
        </div>
      )}

      <ResponsiveSheet open={!!selected} onOpenChange={(o) => !o && setSelectedId(null)}>
        <ResponsiveSheetContent size="lg">
          {selected && <ProjectSheet p={selected} />}
        </ResponsiveSheetContent>
      </ResponsiveSheet>
    </div>
  );
