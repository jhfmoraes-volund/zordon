...
// ClientInsightsCard
//
// Aggregate Alpha read of a client across all of their projects. Same two
// blocks as the project-level card (Relacional + Técnico), but the LLM was
// prompted to consolidate signals from every project of the client.
//
// Renders nothing useful to viewers/contributors — RLS gates it on
// is_manager(). The parent (client page) also hides this when the caller is
// below manager, so unauthorized requests never even fire.
//
// The per-project drill-down lives inside ClientProjectCard (see
// project-insight-chips.tsx).
...
type LoadResponse = {
  ...
  pendingJob: Job | null;
  projects: Project[];
  projectInsights: ProjectInsightMini[];
};
...
export function ClientInsightsCard({ clientId }: { clientId: string }) {
  ...
  useEffect(() => {
    ...
    const channel = client
      .channel(`client-insights:${clientId}`)
      .on(
        "postgres_changes",
        {
          event: "*",
  ...
  function rerun() {
    startTransition(async () => {
      const res = await fetch(`/api/clients/${clientId}/insights/rerun`, {
        method: "POST",
      });
      if (res.ok) {
        toast.success("Atualizando insights do cliente…");
        load();
        return;
      }
    ...
    return (
      <Card size="sm" className="px-4 py-4">
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-sm font-semibold">Alpha Insights · Cliente</h2>
        ...
        <p className="text-xs text-muted-foreground">
          Alpha ainda não analisou este cliente. A próxima execução automática roda às 07:00.
        </p>
      </Card>
    );
  }
  ...
  return (
    <Card size="sm" className="px-4 py-4">
      ...
      <p className="text-[11px] text-muted-foreground">
        Base: {insight.inputProjectsCount} projeto(s) · {insight.inputMeetingsCount} reunião(ões) nos últimos 14 dias.
      </p>
    </Card>
  );
}

// ─── Block (mirror of AlphaInsightsCard's) ────────────────────────────────
