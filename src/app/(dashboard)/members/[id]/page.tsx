...
import {
  ...
  type ProjectView,
  type WeekView,
} from "./_components/management";
import { InsightsTab, type InsightWeekDone, type InsightWeekPlan } from "./_components/insights-tab";
import {
  FLAG_RANK,
  OK_GREEN,
  WARN_RED,
...
export default function MemberCapacityPage() {
  ...
  const [doneWeeks, setDoneWeeks] = useState<InsightWeekDone[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!capRes.ok) {
        const d = await capRes.json().catch(() => ({ error: capRes.statusText }));
  ...
  // default selecionado = primeiro da ordem por carga
  useEffect(() => {
