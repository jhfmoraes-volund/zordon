...
export function buildIngestSeed(
  ...
  if (meetingType === "private") {
    const lines = [
      ...
      `2) Persista o texto BRUTO via \`save_meeting_transcript_text\` (source="${source}", sourceId="${sourceId}", fullText=<texto do passo 1>) — isso grava no TranscriptRef (SSOT) pra reuso futuro.`,
      "3) Escreva um RESUMO conciso da transcrição no campo `notes` da Meeting (use update_meeting_notes com notes=<resumo>).",
