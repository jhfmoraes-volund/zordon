...
/** Atomically claim a single pending job. Returns null if none. */
export async function claimNextJob(admin: Client, jobId?: string): Promise<ClaimedJob | null> {
  if (jobId) {
    const { data } = await admin
      ...
      .eq("id", jobId)
      .eq("status", "pending")
      .select("id, projectId, triggeredByMemberId, source")
      .single();
...
export async function runInsightJob(
  ...
  let ctx;
  try {
    ctx = await loadInsightContext(admin, job.projectId);
  } catch (e) {
